...
abstract class DatabaseModel
{
	...
	public function __construct($input = null)
	{
		...
		if(is_numeric($input) && $input > 0 ){
			// if input is a number, load the record from the db
			$this->find($input);
		}
		if (is_array($input)) {
	...
	{
		if(! self::$db){
			$dsn ='mysql:host=localhost;dbname=se7enjays;charset=utf8';
	...
	public static function all($sortcolumn = "", $asc = true, $pageNumber=null, $pageSize=null)
	{
		...
		if($sortcolumn){
			if( ! array_search($sortcolumn, static::$columns)){
				throw new UnexpectedValueException("Property $sortcolumn is not found in the column array");
			}
	...
	public static function allBy($column, $value, $sortcolumn = "", $asc = true, $pageNumber=null, $pageSize=null)
	{
		...
		if( ! array_search($column, static::$columns)){
			throw new UnexpectedValueException("Propert $column is not found in the column");
		}
